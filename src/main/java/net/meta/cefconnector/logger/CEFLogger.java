...
public class CEFLogger {
    ...
    /*
        Description: This function will take responseData string and convert it to CEF format. 
        If successful, it will return the offset token
    ...
    */
    ...
    public String saveLogs(String responseData) {
        ...
        //if the responseData is null for some reason, return a null token offset
        if (responseData == null) {
            return null;
        }

        //parse the response data string by newlines into individual JSONObjects
        ...
        /*
            loop through the JSONObjects and attempt to process them one by one into CEF format 
            This will process up to the last line which contains the offset token
        */
        for(int ctrl =0; ctrl<tokenCount-1;ctrl++){
            ...
            try{
                ...
                /*
                    Attempt to convert the JSON Formatted string into a valid JSONObject and call prepareParams
                    prepareParams returns a map which will contain the CEF header as the first key and then a 
                ...
                */
                ...
                //loop through the map one by one and retrieve the key/value maps individually
                for (Map.Entry<String, String> entry : params.entrySet()) {
                    
                    String val = entry.getValue();
                    
                    //If the key equals CEFHeader than this value contains the full CEF Header
                    if((!val.equals("")) && ((entry.getKey().equals("CEFHeader")))){
                        logLineHeader = val;
                    }
                    //Otherwise the key contains the cef tag and the value contains the value for the cef tag
                    else if ((!val.equals(""))) {
                        logLine.append(entry.getKey() + "=" + val + " ");
                    }
                }

                //Send the full CEF Header and CEF extension message to be logged to the SIEM. Remove any trailing spaces.
                LogSaver.save((logLineHeader+logLine.toString()).replaceAll("\\s+$", ""));
            }//if the current responseData  newline is not a proper JSONObject, skip and log to error
        ...
        //This will contains the last line of the responseData, which will contain the offset token
        line = st.nextToken();
        if (line != null) {
        ...
        //after all the security events have been proceed, return the offset token
        return token;
    }
  
    /*
        Description: This function will process and store the components of the CEF log into a map key/val pair
    ...
    */
    ...
    private Map<String, String> prepareParams(JSONObject rec) {
        ...
        /*
            The order of the following is important since they don't have a field key. 
            They need to in this exact order to meet CEF standards.
        */
        
        //Get User defined format. This will map akamai api values to defined extension attributes
        ...
        for(int ctrl=0; ((!CEFFormatExtension.equals(""))&&(ctrl < CEFFormatExtensionArray.length)); ctrl++){
            ...
            if(mCalculated.find()) // Calculated Akamai API variable defined by $. Need to find Value from function
            {
            ...
            else if(mAPI.find()) // Dynamic Akamai API variable defined by ${}. Need to find Value
            {
                try{
    
                    JSONObject tempObject = null; 
                    String[] paramDefinition = mAPI.group(1).split("\\."); // separate API Variable constructs 
            ...
            else if(mStatic.find()) // Pull Static Value Defined in Properties. Needs to be between double quotes
            {
        ...
        */
        ...
        for(int ctrl=0; ctrl < CEFFormatHeaderArray.length; ctrl++){
            ...
            if(mCalculated.find()) // Calculated Akamai API variable defined by $. Need to find Value
            {
                String function = mCalculated.group(1); //function name
                String tempString = "";
            ...
            else // Pull Static Value Defined in Properties. Needs to be between double quotes
            {
    ...
    */
    ...
    /*
        Description: Verifies if the current JSONOBject key is in the list of base64encoded values
                     If it is, then return true, otherwise return false
        Arguments: String attribute, String[] base64Fields
        Return: boolean
    */
    ...
    /*
        Description: Verifies if the current JSONOBject key is in the list of URL encoded values
                     If it is, then return true, otherwise return false
        Arguments: String attribute, String[] URLEncodedFields
        Return: boolean
    */
    ...
    /*
        Description: base64decode the current base64encoded string
    ...
    */
    private String base64Decode(String value){
        ...
        //the current string is not a valid base64encoded string. Log to warn and return original value
    ...
    /*
        Description: Convert epoch time to millseconds
        Arguments: String epoch
        Return: String
    */
    ...
    /*
        ...
        if attackData.ruleActions contains any action other than 'alert' or 'deny',
        use it (there will be only one)
        otherwise, check if there is a 'deny' action and if so, set the applied
        action to 'deny'
        otherwise, check if attackData.slowPostAction equals to 'A', set the
        applied action to 'abort'
    ...
    */
    private String appliedAction(JSONObject rec){
        ...
        if((!decoded.contains("alert"))&& !(decoded.contains("deny"))){
            ...
            pAction = "deny";
        else if(rec.has("attackData") && rec.getJSONObject("attackData").has("slowPostAction") && rec.getJSONObject("attackData").getString("slowPostAction").equals("A"))
            pAction = "abort";
        else
            pAction = "alert";
    ...
    */
    ...
    /*
        Description: Calculate CEF header name based on eventClassId
        Arguments: JSONObject rec
        Return: String
    */
    ...
    */
    ...
    /*
        Description: If there is attackData with a clientIP, return ipv6src if it's in the correct format.
        Arguments: JSONObject rec
        Return: String
    */
    ...
    /*
        Description: Based on the function name retrieved from the properties file, call available function
        Arguments: JSONObject rec, String function
        Return: String
    */
