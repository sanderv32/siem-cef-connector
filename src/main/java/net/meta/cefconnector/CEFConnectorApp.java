		...
		while (!stopped) {
			try {
				...
				try {
					...
					if (statusCode == 200) {
						BlockingQueue<Message> queue = new LinkedBlockingQueue<Message>(1024 * 2);
						EventConsumer consumerTask = new EventConsumer(context, new CEFLogger(), queue);
						...
						try (BufferedReader is = new BufferedReader(
								new InputStreamReader(response.getEntity().getContent()), 1024 * 16)) {
